...
namespace CSArp
{
    public class Controller
    {
        ...
        {
            MessageBox.Show("Autor original : globalpolicy\nContacto : yciloplabolg@gmail.com\nBlog : c0dew0rth.blogspot.com\nGithub : globalpolicy\n¡Todos los créditos son para él!", "Sobre ByeWifi", MessageBoxButtons.OK);
        }
        ...
        public void ToolStripSaveClicked()
        {
            if (ApplicationSettingsClass.SaveSettings(_view.ListView1, _view.ToolStripComboBoxDeviceList.Text))
                _view.ToolStripStatus.Text = "Configuración guardada";
        }
            ...
            _view.SaveFileDialogLog.FileOk += (object sender, System.ComponentModel.CancelEventArgs e) =>
            {
                if(_view.SaveFileDialogLog.FileName!="" && !File.Exists(_view.SaveFileDialogLog.FileName))
                {
                    try
                    {
                        ...
                        DebugOutputClass.Print(_view, "Registro guardado en " + _view.SaveFileDialogLog.FileName);
                    }
                    catch(Exception ex)
                    {
